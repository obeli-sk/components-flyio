use crate::exports::activity_flyio::fly_http::secret;
...
async fn list_secrets(app_name: String) -> Result<Vec<secret::Secret>, anyhow::Error> {
...
async fn put_secret(
    ...
    secret_name: String,
    value: String,
) -> Result<secret::Secret, anyhow::Error> {
    #[derive(Serialize)]
    struct PutBody {
        value: String,
    }
...
impl secret::Guest for crate::Component {
    /// List all secrets for a given app.
    fn list(app_name: String) -> Result<Vec<secret::Secret>, String> {
        block_on(list_secrets(app_name)).map_err(|err| err.to_string())
    }

    /// Set a secret for a given app.
    fn put(app_name: String, secret_name: String, value: String) -> Result<secret::Secret, String> {
